...
class TogglePause(tables.BatchAction):
    ...
    icon = "pause"
    classes = ("btn-danger",)
    help_text =_("This action will pause the instances,you can recover it by 'cover instances'.")
...
class ToggleSuspend(tables.BatchAction):
    name = "suspend"
    classes = ("btn-suspend","btn-danger")
    help_text =_("This action saves memory data into the disk and power off,then you can recover it by 'cover instances'.")
    @staticmethod
    def action_present(count):
        return (
            ungettext_lazy(
...
class ConfirmResize(policy.PolicyTargetMixin, tables.BatchAction):
    name = "confirm"
    verbose_name = _("Confirm Resize/Migrate")
    classes = ("btn-confirm", "btn-action-required",'btn-danger')
    policy_rules = (("compute", "compute:confirm_resize"),)
    help_text = _("Terminated instances are not recoverable.")
    @staticmethod
    def action_present(count):
...
#        api.nova.server_confirm_resize(request, instance)
    ...
    def action(self, table, request, instance):
        api.nova.server_confirm_resize(request, instance)
'''
